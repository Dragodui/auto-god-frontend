...
import { fetchItemById, purchaseItem } from '../store/actions/itemActions';
...
const ItemDetail: React.FC = () => {
  ...
  const { currentItem: item, loading, error } = useSelector((state: RootState) => state.items);
  ...
  if (error || !item) {
    return <div className="text-red-500 text-center">{error || 'Item not found'}</div>;
  }
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="border-t pt-4">
            ...
            <p>Name: {item.seller.name} {item.seller.lastName}</p>
